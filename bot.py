...
ADMIN_ID = 7984990535
CRYPTO_TOKEN = "576779:AA68SugHnwHzOeKMEJDr0cKggB1ThAAwQmp"
...
def slots_ask(message):
bot.send_message(message.chat.id, "🎰 Введи ставку (от 5 RUB):\nНапример: /slot 10")
...
bot.send_message(message.chat.id, "❌ Пример: /slot 10")
return
if bet < 5:
bot.send_message(message.chat.id, "❌ Минимальная ставка: 5 RUB")
return
...
bot.send_message(message.chat.id, "❌ Пример: /roulette 50")
return
if bet < 5:
bot.send_message(message.chat.id, "❌ Минимальная ставка: 5 RUB")
return
...
print("SWILL CASINO v5.0 STARTED")
